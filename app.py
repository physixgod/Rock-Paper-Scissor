...
def start_hand_detection():
    ...
    detector = mp.handDetector(detectionCon=int(0.75))
    tipIds = [4, 8, 12, 16, 20]
...
def get_scores():
    global my_score
    global pc_score
    return jsonify({"my_score": my_score, "pc_score": pc_score})
@app.route('/video_feed')
def video_feed():
    return Response(start_hand_detection(),mimetype='multipart/x-mixed-replace; boundary=frame')
if __name__ == "__main__":
    app.run(debug=True, port=5000)
    if game_ended:
        # Wait for 5 seconds before terminating the program
        time.sleep(5)
